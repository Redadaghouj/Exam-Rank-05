...
int	parse_header(char* line, int len, t_map* m)
{
	int	i = 0;
...
int	save_row(t_map* m, char* line, int len, int y)
{
	if (y == 0)
	{
		m->w = len;
...
void	keep_best(t_map* m, int y, int x, int size)
{
	if (size > m->best)
	{
		m->best = size;
